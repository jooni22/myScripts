...
all=$(wget -qO- http://jooni22.usermd.net/mining/index.php | grep -v "http://" | grep "[192]\.[168]\.[0-1]" | cut -d "<" -f 1,4 | sed -e 's/<td>/ /' | awk '!a[$0]++' > all.txt)
...
host="host"
rack="rack"
hostname="(/opt/ethos/sbin/ethos-readdata hostname)"
rack_loc="(cat /home/ethos/remote.conf)"

#wykonuje polecenia
sudo sshpass -p live parallel-ssh -h allip.txt -l ethos -t 7 -O StrictHostKeyChecking=no -O UserKnownHostsFile=/dev/null -A -P "host=$dolar$hostname; rack=$dolar$rack_loc; echo $dolar$rack $dolar$host" > result.txt
